...
class plxMotor
{
	...
	public function getArticles($publi = 'before')
	{
		...
		if ($aFiles = $this->plxGlob_arts->query($this->motif, 'art', $this->tri, $start, $this->bypage, $publi)) {
			...
			$artsList = array();
			$realArticlesPath = realpath(PLX_ROOT . $this->aConf['racine_articles']);
			foreach ($aFiles as $v) {
				$filePath = PLX_ROOT . $this->aConf['racine_articles'] . $v;
				$realFilePath = realpath($filePath);

				if ($realFilePath && strpos($realFilePath, $realArticlesPath) === 0) {
					$art = $this->parseArticle($realFilePath);
					if (!empty($art)) {
						$artsList[] = $art;
					}
				}
	...
	public function parseCommentaire($filename)
	{
		...
		$realBasePath = realpath(PLX_ROOT . $this->aConf['racine_commentaires']);
		$realFilePath = realpath($filename);

		if ($realFilePath && strpos($realFilePath, $realBasePath) === 0 && file_exists($realFilePath)) {
			# Mise en place du parseur XML
			$data = implode('', file($realFilePath));
			$parser = xml_parser_create(PLX_CHARSET);
			xml_parser_set_option($parser, XML_OPTION_CASE_FOLDING, 0);
			xml_parser_set_option($parser, XML_OPTION_SKIP_WHITE, 0);
			xml_parse_into_struct($parser, $data, $values, $iTags);
			...
			# Informations obtenues en analysant le nom du fichier
			$tmp = $this->comInfoFromFilename(basename($realFilePath));
			...
			throw new Exception("Chemin de fichier invalide ou fichier inexistant");
		}
	}
	...
	public function nextIdArtComment($idArt)
	{
		...
		$ret = '0';
		if ($dh = opendir(basename(PLX_ROOT . $this->aConf['racine_commentaires']))) {
			$Idxs = array();
	...
	public function addCommentaire($content)
	{
		...
		// Nettoyer et valider le nom de fichier pour éviter les attaques de traversée de chemin
		$filename = basename($content['filename']);
		$filepath = realpath(PLX_ROOT . $this->aConf['racine_commentaires'] . $filename);

		// Vérifier que le chemin est valide et qu'il se trouve dans le répertoire attendu
		if ($filepath === false || strpos($filepath, realpath(PLX_ROOT . $this->aConf['racine_commentaires'])) !== 0) {
			throw new Exception("Chemin de fichier invalide");
		}

		return plxUtils::write($xml, $filepath);
	}
	...
	public function sendTelechargement($cible)
	{
		...
		$decryptedFile = plxEncrypt::decryptId($cible);
		$file = PLX_ROOT . $this->aConf['medias'] . $decryptedFile;

		# Hook plugins
		if (eval($this->plxPlugins->callHook('plxMotorSendDownload'))) return;

		# On lance le téléchargement et on check le répertoire medias
		$realMediaPath = realpath(PLX_ROOT . $this->aConf['medias']);
		$realFilePath = realpath($file);

		if ($realFilePath && strpos($realFilePath, $realMediaPath) === 0 && file_exists($realFilePath)) {
