 ...
 *
 * @package PLX
 * @autor Stephane F, Pedro "P3ter" CADETE
 **/
 ...
 if (isset($_POST['folder']) && $_POST['folder'] != '.' && !plxUtils::checkSource($_POST['folder'])) {
	 $_POST['folder'] = '.';
 }
 ...
 if (empty($_SESSION['medias'])) {
	 ...
	 $_SESSION['folder'] = $_POST['folder'] == '.' ? '' : basename($_POST['folder']);
 }
 
 # Nouvel objet de type plxMedias
 $plxMediasRoot = PLX_ROOT . $_SESSION['medias'];
 if ($plxAdmin->aConf['userfolders'] && $_SESSION['profil'] == PROFIL_WRITER) {
	 $plxMediasRoot .= basename($_SESSION['user']) . '/';
 }
 
 $folder = isset($_SESSION['folder']) ? basename($_SESSION['folder']) : '';
 
 # Vérification que le chemin est bien dans le répertoire des médias
 $realPlxMediasRoot = realpath($plxMediasRoot);
 $realFolderPath = realpath($plxMediasRoot . '/' . $folder);
 
 if ($realFolderPath && strpos($realFolderPath, $realPlxMediasRoot) === 0) {
	 $plxMedias = new plxMedias($plxMediasRoot, $folder, $plxAdmin->aConf['default_lang']);
 } else {
	 # Gestion de l'erreur si le chemin est invalide
	 die('Chemin invalide détecté.');
 }

#----

if (!empty($_POST['btn_newfolder']) && !empty($_POST['newfolder'])) {
    $newFolder = basename($_POST['newfolder']);
    if ($plxMedias->newDir($newFolder)) {
        $_SESSION['folder'] = $_SESSION['folder'] . $newFolder . '/';
    }
    header('Location: medias.php');
    exit;
}

if (!empty($_POST['btn_renamefile']) && !empty($_POST['newname'])) {
    $oldName = basename($_POST['oldname']);
    $newName = basename($_POST['newname']);
    $plxMedias->renameFile($oldName, $newName);
    header('Location: medias.php');
    exit;
}

elseif (!empty($_POST['folder']) && $_POST['folder'] != '.' && !empty($_POST['btn_delete'])) {
    $folderToDelete = basename($_POST['folder']);
    if ($plxMedias->deleteDir($folderToDelete)) {
        $_SESSION['folder'] = '';
    }
    header('Location: medias.php');
    exit;
...
elseif (isset($_POST['selection']) && (!empty($_POST['btn_ok']) && $_POST['selection'] == 'delete') && isset($_POST['idFile'])) {
    $filesToDelete = array_map('basename', $_POST['idFile']);
    $plxMedias->deleteFiles($filesToDelete);
    header('Location: medias.php');
    exit;
}

elseif (isset($_POST['selection']) && (!empty($_POST['btn_ok']) && $_POST['selection'] == 'move') && isset($_POST['idFile'])) {
    $filesToMove = array_map('basename', $_POST['idFile']);
    $destinationFolder = basename($_POST['folder']);
    $plxMedias->moveFiles($filesToMove, $_SESSION['currentfolder'], $destinationFolder);
    header('Location: medias.php');
    exit;
}

elseif (isset($_POST['selection']) && (!empty($_POST['btn_ok']) && $_POST['selection'] == 'thumbs') && isset($_POST['idFile'])) {
    $filesToThumb = array_map('basename', $_POST['idFile']);
    $plxMedias->makeThumbs($filesToThumb, $plxAdmin->aConf['miniatures_l'], $plxAdmin->aConf['miniatures_h']);
    header('Location: medias.php');
    exit;
}
