...
$help = basename($help); // Nettoyage du type d'aide

# page
$page = isset($_GET['page']) ? urldecode($_GET['page']) : '';
$page = plxUtils::nullbyteRemove($page);
$page = basename($page); // Nettoyage de la page
...
if ($filename && strpos($filename, realpath(PLX_PLUGINS)) === 0 || strpos($filename, realpath(PLX_ROOT . $plxAdmin->aConf['racine_themes'])) === 0) {
    if (is_file($filename)) {
        ob_start();
        try {
            echo '
