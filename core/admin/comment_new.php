...
<form action="comment_new.php?<?php echo plxUtils::strCheck($get) ?>" method="post" id="form_comment">
	...
	<div class="inline-form action-bar">
		<h2><?php echo htmlentities(L_CREATE_NEW_COMMENT, ENT_QUOTES, 'UTF-8') ?></h2>
		<?php if(!empty($_GET['a'])) : ?>
		<p><a class="back" href="comments.php?a=<?php echo htmlentities($_GET['a'], ENT_QUOTES, 'UTF-8'); ?>"><?php echo L_BACK_TO_ARTICLE_COMMENTS ?></a></p>
		<?php else : ?>
	...
	<h3 class="no-margin"><?php echo L_COMMENT_LINKED_ARTICLE_FIELD ?>&nbsp;:&nbsp;<?php echo htmlentities($article, ENT_QUOTES, 'UTF-8') ?></h3>
	...
	<fieldset>
		<div class="grid">
			<div class="col sml-12">
				...
				<?php plxUtils::printInput htmlentities(('parent',$parent,'hidden', ENT_QUOTES, 'UTF-8')); ?>
		...
		<div id="c<?php echo $comId ?>" class="comment<?php echo ((isset($_GET['c']) AND $_GET['c']==$comId)?' current':'') ?> level-<?php echo $plxAdmin->plxRecord_coms->f('level'); ?>">
			<div id="com-<?php echo $plxAdmin->plxRecord_coms->f('index'); ?>">
				<small>
					...
					- <a href="comment.php<?php echo htmlentities(!empty($_GET['a'], ENT_QUOTES, 'UTF-8'))?'?c='.$comId.'&amp;a='.$_GET['a']:'?c='.$comId; ?>" title="<?php echo L_COMMENT_EDIT_TITLE ?>"><?php echo L_COMMENT_EDIT ?></a>
